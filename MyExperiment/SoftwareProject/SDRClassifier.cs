...
namespace MyExperiment.SEProject
{
    public class SdrClassifier
    {
        ...
        /// </summary>
        public FlexComRowMatrixUtility<object> WeightMatrixUtility;
        ...
        private void InitializeEntries()
        {
            ...
            WeightMatrixUtility = new FlexComRowMatrixUtility<object>();
        ...
        public double[] Predict(int[] patternNz)
        {
            ...
            {
                throw new Exception("empty bucket exception");
            }
        ...
        private void UpdateWeightMatrix(List<object> classification)
        {
            foreach (Tuple<int, object> t in patternNzHistory)
            {
                ...
                for (int row = 0; row <= maxBucketIdx; row++)
                {
                    ...
                    {
                        WeightMatrixUtility.AddAndUpdate(row, bit, alpha * error[row]);
                    }
                }
            }
        }
        ...
        private double[] inferSingleStep(int[] patternNz)
        {
            ...
            for (int row = 0; row <= maxBucketIdx; row++)
            {
                ...
                {
                    outputActivationSum[row] += Convert.ToDouble(WeightMatrixUtility.Matrix[row][bit]);
                }
            }
        ...
        private void GrowMatrixUptoMaximumInput(int newMaxInputIdx)
        {
            for (int i = 0; i < maxBucketIdx + 1; i++)
            {
                ...
                try
                {
                    inputList = WeightMatrixUtility.Matrix[i];
                ...
                catch (ArgumentOutOfRangeException ex)
                {
                    ...
                    WeightMatrixUtility.Matrix.Add(inputList);
                    continue;
                }

                AddZeros(maxInputIdx, newMaxInputIdx - 1, inputList);
                WeightMatrixUtility.Matrix[i] = inputList;
            }
        }
        ...
        private void AddBucketsToWeightMatrix(int bucketIdx)
        {
            for (int i = maxBucketIdx; i < bucketIdx; i++)
            {
                ...
                WeightMatrixUtility.Matrix.Add(list);
            }
        }
        ...
        public void PrintWeightMatrix()
        {
            ...
            for (int i = 0; i < maxBucketIdx + 1; i++)
            {
                for (int j = 0; j < maxInputIdx + 1; j++)
                {
                    Console.Write(WeightMatrixUtility.Matrix[i][j] + " ");
                }
